...
const storeToken = async ({ userId, publicKey, refreshToken }: TokenSchema) => {
  const existedUser = await findKeyById({ userId: userId });
...
const findKeyById = async ({ userId }: { userId: string }) => {
